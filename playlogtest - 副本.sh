...
transferFromBosstoTerminal()
{
	...
	listunspent=`bitcoin-cli -regtest listunspent 1 9999999 "[\"${bossAddr}\"]" | jq '.'`
...
transfer()
{
	...
	listunspent=`bitcoin-cli -regtest listunspent 1 9999999 "[\"${add2}\"]" | jq '.'`
	# echo listunspent=$listunspent
	
	# 获取add2的未花费交易号,vout,余额,这个是看是否存在
	txid=`bitcoin-cli -regtest listunspent 1 9999999 "[\"${add2}\"]" | jq '.[0].txid'`
	echo txid=$txid
	vout=`bitcoin-cli -regtest listunspent 1 9999999 "[\"${add2}\"]" | jq '.[0].vout'`
	echo vout=$vout
	
	# 余额
	amount=`bitcoin-cli -regtest listunspent 1 9999999 "[\"${add2}\"]" | jq '.[0].amount'`
	echo amount=$amount 
	
	# 这里判断能否转账
	# 如果 txid 和 vout 没有即没有listunspent的结果或者余额达不到最小的要求，均是请求总地址来转钱
	if [ "$txid" = "null" ] || [ `expr $amount \< $terminalMinBalance` -eq 1 ];then
	...
	do
		...
		moneyAndFee=`echo "$singleTransferMoneyFromTerminalToAd + $myFee" | bc`
		if [ `echo "$unspent >= $moneyAndFee" | bc` -eq 1 ]
		then
			echo "$index : $unspent  -> 金额大小足够，可以给广告转"
			...
			#rechange=$(printf "%.2f" `echo "scale=2;$amount - $moneyAndFee"| bc`)
			# 计算找零
			rechange=`echo "$amount - $moneyAndFee" | bc -l`
